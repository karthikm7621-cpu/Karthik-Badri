...
@hr_bp.route("/request-join", methods=["POST"])
...
@hr_bp.route("/sync-attendance", methods=["POST"])
...
@hr_bp.route("/ask-policy", methods=["POST"])
def ask_policy() -> Any:
...
@hr_bp.route("/verify-attendance", methods=["POST"])
...
@hr_bp.route("/submit-audio-leave", methods=["POST"])
...
@hr_bp.route("/submit-receipt", methods=["POST"])
...
@hr_bp.route("/upload-resume", methods=["POST"])
...
@hr_bp.route("/submit-hr-ticket", methods=["POST"])
...
@hr_bp.route("/pending-users", methods=["GET"])
...
@hr_bp.route("/approve-user", methods=["POST"])
...
@hr_bp.route("/delegate-owner", methods=["POST"])
...
@hr_bp.route("/add-stream", methods=["POST"])
def add_stream() -> Any:
    auth_user = resolve_authenticated_user()
    if not auth_user or not is_owner(auth_user):
        return jsonify({"success": False, "message": "Owner access required."}), 403
